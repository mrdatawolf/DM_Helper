...
import { visitedShadowCardStyle, visitedInfluenceLabel } from './player-shadows.js';
...
async function loadCreatures() {
    ...
    try {
        allCreatures = await apiFetch('/api/npcs');
        renderCreatures();
    } catch (err) {
...
function renderCreatures() {
    ...
    container.innerHTML = Object.entries(byRole).map(([role, creatures]) => `
        <div style="grid-column:1/-1;margin:12px 0 4px"><h3 style="margin:0">${escHtml(role)}</h3></div>
...
function creatureCardHtml(n, showSpoilers) {
    ...
    <div class="creature-player-card" style="${style}">
        ...
        <span class="creature-role-badge">${escHtml(n.role || 'Creature')}</span>
        ${s.size_type ? `<p class="shadow-depth-flavor">${escHtml(s.size_type)}${n.alignment ? `, ${escHtml(n.alignment)}` : ''}</p>` : ''}
        ${n.description ? `<p class="creature-desc">${escHtml(n.description)}</p>` : ''}
        <div style="display:flex;align-items:center;gap:10px;margin:8px 0;flex-wrap:wrap">
            ...
            ${s.speed ? `<span style="font-size:0.8rem;color:#888">${escHtml(s.speed)}</span>` : ''}
        </div>
        ${abilityRow ? `<p style="font-size:0.82rem;color:#888;margin:0 0 10px">${abilityRow}</p>` : ''}
        ${(s.traits || []).length ? `<p style="margin:6px 0 2px"><strong style="font-size:0.85rem">Traits</strong></p><ul style="margin:0 0 8px;padding-left:18px;font-size:0.88rem;color:#555">${s.traits.map(t => `<li>${escHtml(t)}</li>`).join('')}</ul>` : ''}
        ${(s.lore || []).length ? `<p style="margin:6px 0 2px"><strong style="font-size:0.85rem">Lore</strong></p><ul style="margin:0;padding-left:18px;font-size:0.88rem;color:#555">${s.lore.map(t => `<li>${escHtml(t)}</li>`).join('')}</ul>` : ''}
    </div>`;
...
Object.assign(window, { enableCreatureSpoilers, toggleCreatureSpoilers });

// Used by player-core.js.
export { loadCreatures, syncCreatureSpoilerButton };
