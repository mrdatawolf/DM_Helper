...
router.get('/:id', authenticate, (req, res) => {
    try {
        ...
        res.json({
            ...
            gear,
            powers,
            familiars: familiars.map(f => serializeFamiliar(f, character.level, !!req.user?.isDM)),
...
router.get('/:id/familiars', authenticate, (req, res) => {
    try {
        const db = getDatabase();
        ...
        const familiars = db.prepare('SELECT * FROM familiars WHERE character_id = ? AND is_active = 1').all(req.params.id);
        res.json(familiars.map(f => serializeFamiliar(f, character.level, req.user.isDM || req.user.isAdmin)));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/:id/familiars', authenticate, (req, res) => {
    try {
        if (!requireDMUser(req, res)) return;
        const db = getDatabase();
...
router.put('/:id/familiars/:familiarId', authenticate, (req, res) => {
    try {
        const db = getDatabase();
        const character = db.prepare('SELECT id, level, user_id FROM characters WHERE id = ?').get(req.params.id);
        if (!character) return res.status(404).json({ error: 'Character not found' });
...
router.delete('/:id/familiars/:familiarId', authenticate, (req, res) => {
    try {
        if (!requireDMUser(req, res)) return;
        const db = getDatabase();
